                                ...
                                <td>
                                    @if(empty($getStudentAnswer))
                                        @if($getAssignmentCompletion->count() == 0)
                                            ...
                                            @else
                                                <form method="post" action="{!! URL::to('/') !!}/myCourse/enterClass/{!! $idCoursesClass !!}/enterSession/{!! $idTopic !!}/{!! $idSubTopic !!}/enterAssignment/{!! $idAssignment !!}/written/{!! $question->idQuestion !!}/submitAssignmentAnswer">
                                                    <input type="hidden" name="_token" value="{{ csrf_token() }}">
                                                    <div class="form-group">
                                                        <label class="control-label col-sm-4" for="email">Your Answer :</label>
                                                        <div class="col-sm-12">
                                                            <textarea class="form-control" id="answer{!! $question->idQuestion !!}" name="answer{!! $question->idQuestion !!}"></textarea>
                                                            <script>
                                                                CKEDITOR.replace( 'answer{!! $question->idQuestion !!}');
                                                            </script>
                                                        </div>
                                                    </div>
                                                    <div class="form-group">
...
<script language="JavaScript">
    function completeAssignment(){
        ...
        if(isConfirm){
            location.href='/myCourse/enterClass/{!! $idCoursesClass !!}/enterSession/{!! $idTopic !!}/{!! $idSubTopic !!}/enterAssignment/{!! $idAssignment !!}/written/completeAssignment';
        }
    }
</script>
