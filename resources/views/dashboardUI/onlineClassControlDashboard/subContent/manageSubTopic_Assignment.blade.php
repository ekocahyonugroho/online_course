                        ...
                        @foreach($dataAssignment AS $data)
                            ...
                            <tr>
                                ...
                                <td>
                                    <table border="0">
                                        <tr>
                                            <td><button onclick="deleteAssignment({!! $data->idAssignment !!},'{!! $data->assignmentType !!}')" class="btn btn-danger"><i class="fa fa-trash" aria-hidden="true"></i></button></td>
                                            <td><button onclick="editAssignment({!! $data->idAssignment !!},'{!! $data->assignmentType !!}')" class="btn btn-warning"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></button></td>
                                            <td><button onclick="evaluateAssignment({!! $data->idAssignment !!},'{!! $data->assignmentType !!}')" class="btn btn-info">{!! $getUnevaluatedAssignment->count() !!} Students Completed</button></td>
                                        </tr>
                                    </table>
                                </td>
                                <td>{!! $creatorName !!}</td>
...
<script language="JavaScript">
    function editAssignment(idAssignment,typeAssignment){
        location.href = '/manageOnlineCourse/availableClass/manageOnlineClass/{!! $idCoursesClass !!}/manageSession/{!! $idTopic !!}/{!! $idSubTopic !!}/manageAssignment/'+idAssignment+'/'+typeAssignment+'/editAssignment';
    }

    function deleteAssignment(idAssignment,typeAssignment){
        var isConfirm = confirm('Are you sure to delete this Assignment?');
        if(isConfirm) {
            location.href = '/manageOnlineCourse/availableClass/manageOnlineClass/{!! $idCoursesClass !!}/manageSession/{!! $idTopic !!}/{!! $idSubTopic !!}/manageAssignment/' + idAssignment + '/' + typeAssignment + '/deleteAssignment';
        }
    }

    function evaluateAssignment(idAssignment,typeAssignment){
        location.href = '/manageOnlineCourse/availableClass/manageOnlineClass/{!! $idCoursesClass !!}/manageSession/{!! $idTopic !!}/{!! $idSubTopic !!}/manageAssignment/'+idAssignment+'/'+typeAssignment+'/evaluateAssignment';
    }
</script>
