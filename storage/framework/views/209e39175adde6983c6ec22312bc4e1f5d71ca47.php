                                ...
                                <td>
                                    <?php if(count($getStudentAnswer) == 0): ?>
                                        <?php if($getAssignmentCompletion->count() == 0): ?>
                                            ...
                                            <?php else: ?>
                                                <form method="post" enctype="multipart/form-data" action="<?php echo URL::to('/'); ?>/myCourse/enterClass/<?php echo $idCoursesClass; ?>/enterSession/<?php echo $idTopic; ?>/<?php echo $idSubTopic; ?>/enterAssignment/<?php echo $idAssignment; ?>/upload/<?php echo $question->idQuestion; ?>/submitAssignmentAnswer">
                                    ...
                                    <?php else: ?>
                                        <form>
                                            <div class="form-group">
                                                <label class="control-label col-sm-4" for="email">Your Answer (Max  <?php echo ini_get('post_max_size'); ?> bytes) :</label>
                                                <div class="col-sm-6">
                                                    <button type="button" onclick="location.href='<?php echo URL::to('/').$getStudentAnswer->answerValue; ?>'" class="btn btn-success">My Answer</button>
                                                </div>
                                            </div>
                                        </form>
                                        <br />
...
<script language="JavaScript">
    function completeAssignment(){
        ...
        if(isConfirm){
            location.href='/myCourse/enterClass/<?php echo $idCoursesClass; ?>/enterSession/<?php echo $idTopic; ?>/<?php echo $idSubTopic; ?>/enterAssignment/<?php echo $idAssignment; ?>/upload/completeAssignment';
        }
    }
</script>
