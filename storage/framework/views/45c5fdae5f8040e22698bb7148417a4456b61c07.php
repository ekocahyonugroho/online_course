...
<ol class="chat">
    <?php $__currentLoopData = $stmtPrivateMessageContents->get(); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $dataPrivateMessage): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
        <?php
        ...
        <?php if($dataPrivateMessage->idMemberFrom == session('idMember')): ?>
            <li class="self">
        <?php else: ?>
            <li class="other">
        <?php endif; ?>
            <div class="avatar"><img src="<?php if(empty($photoMember)): ?> <?php echo asset('images/NO-IMAGE.png'); ?> <?php else: ?> <?php echo asset($photoMember->PhotoDirectory); ?> <?php endif; ?>" draggable="false"/></div>
            <div class="msg">
                <?php echo $dataPrivateMessage->privateMessageContents; ?>

                <time><?php if($dateNow == $dateMessage): ?> <?php echo $timeMessage; ?> <?php else: ?> <?php echo $dataPrivateMessage->dateTime; ?> <?php endif; ?></time>
            </div>
        </li>
    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
</ol>
    <form method="post" action="<?php echo URL::to('/'); ?>/manageOnlineCourse/availableClass/manageOnlineClass/<?php echo $idCoursesClass; ?>/managePrivateMessage/showMessage/<?php echo $idPrivateMessage; ?>/sendReplyPrivateMessage" class="form-horizontal">
        <input type="hidden" name="_token" value="<?php echo e(csrf_token()); ?>">
        <div class="form-group">
            <label class="control-label col-sm-4" for="email">Message :</label>
            <div class="col-sm-12">
                <textarea class="form-control" id="message" name="message"><?php echo e(old('message')); ?></textarea>
            </div>
            <script>
